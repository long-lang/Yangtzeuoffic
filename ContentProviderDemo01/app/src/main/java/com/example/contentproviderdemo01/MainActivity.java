...
public class MainActivity extends AppCompatActivity implements
    ...
    public void onClick(View v) {
        ...
        uri = Uri.parse("content://com.example.createdatabasedemo01/info");
        //新建一个ContentValues对象，该对象以key-values的形式来添加记录到数据库表中
        values = new ContentValues();
        switch (v.getId()) {
            case R.id.btn_insert:
                Random random = new Random();
                values.put("name", "add_hh" + random.nextInt(10));
            ...
            case R.id.btn_update:
                //将数据库info表中name为hh的这条记录更改为name是update_hh
                values.put("name", "update_hh");
