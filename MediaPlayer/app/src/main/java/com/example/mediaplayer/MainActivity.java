...
//歌曲当前播放时长
    ...
    @Override
    protected void onDestroy() {//用户可能不点击exit按钮而直接退出app
        super.onDestroy();
        unbind(isUnbind); //解绑服务
    }
    public void btnClick_play(View v){//播放按钮点击事件
        musicControl.play(); //播放音乐
        animator.start(); //播放动画
    }
    public void btnClick_pause(View v) {//暂停按钮点击事件
        musicControl.pausePlay(); //暂停播放音乐
        animator.pause(); //暂停播放动画
    }
    public void btnClick_continue(View v) { //继续播放按钮点击事件
        musicControl.continuePlay(); //继续播放音乐
        animator.start(); //播放动画
    }
    public void btnClick_exit(View v) { //退出按钮点击事件
